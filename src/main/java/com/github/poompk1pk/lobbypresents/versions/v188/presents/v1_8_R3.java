...
public class v1_8_R3 implements Presents {
	...
	public void PickupSound(Player p) {
		try {
			...
			Main.getInstance().getLogger().warning("Not found sound: ORB_PICKUP on "+Main.getInstance().getVersion());
		}
  ...
  public void FoundSound(Player p, float f) {
    try {
		...
		Main.getInstance().getLogger().warning("Not found sound: "+PresentsUtils.found+" "+Main.getInstance().getVersion());
	}
  }
  
  public void conSoundDefault() {
    if (PresentsUtils.config.get(ConfigType.Default).getConfig().getString("Sounds.found") == null) {
      PresentsUtils.config.get(ConfigType.Default).getConfig().set("Sounds.found", Sound.LEVEL_UP.name());
      PresentsUtils.config.get(ConfigType.Default).save();
    }
	  if (PresentsUtils.config.get(ConfigType.Default).getConfig().getString("Sounds.alreadyfound") == null) {
		  PresentsUtils.config.get(ConfigType.Default).getConfig().set("Sounds.alreadyfound", Sound.CREEPER_DEATH.name());
		  PresentsUtils.config.get(ConfigType.Default).save();
	  }
	  if (PresentsUtils.config.get(ConfigType.Default).getConfig().getString("Effect.click") == null) {
      PresentsUtils.config.get(ConfigType.Default).getConfig().set("Effect.click", EnumParticle.REDSTONE.name());
      PresentsUtils.config.get(ConfigType.Default).save();
    } 
  }
  @Override
