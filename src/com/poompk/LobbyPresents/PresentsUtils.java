...
public class PresentsUtils {
  ...
  public static String getTextNormalMessage(Player p) {
    int total = getProfile(p.getUniqueId()).getClaim().size();
    if (total >= total_present)
      return Message_completed.replace("%player%", p.getDisplayName()).replace("%found%", (new StringBuilder(String.valueOf(getProfile(p.getUniqueId()).getClaim().size()))).toString()).replace("%total%", (new StringBuilder(String.valueOf(total_present))).toString()); 
    return Message_found.replace("%player%", p.getDisplayName()).replace("%found%", (new StringBuilder(String.valueOf(getProfile(p.getUniqueId()).getClaim().size()))).toString()).replace("%total%", (new StringBuilder(String.valueOf(total_present))).toString());
  }
  ...
  public static void CheckVersionSystem(CommandSender sender) {
    try {
      ...
      if (version_this < version_new) {
        sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&c Version outdated You must update LobbyPresents to verion " + version_new + "!"));
        ...
        sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&7 Version: &bThe latest version (" + version_new + ")"));
      } 
    } catch (Exception exception) {}
  }
}
