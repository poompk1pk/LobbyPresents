  ...
  public void onPlayerInteractPresentBoxes(PlayerInteractEvent e) {
    ...
    if (PresentsUtils.getProfile(e.getPlayer().getUniqueId()).getClaim().contains(Integer.valueOf(getId()))) {
      ...
      if (!PresentsUtils.Message_alreadyfound.equalsIgnoreCase("none"))
        PresentsUtils.chat((CommandSender)e.getPlayer(), PresentsUtils.Message_alreadyfound.replace("%player%", e.getPlayer().getDisplayName()).replace("%found%", (new StringBuilder(String.valueOf(PresentsUtils.getProfile(e.getPlayer().getUniqueId()).getClaim().size()))).toString()).replace("%total%", (new StringBuilder(String.valueOf(PresentsUtils.total_present))).toString())); 
      return;
    } 
